...
DEFAULT_LIMIT = 100  # Лимит товаров по умолчанию
...
class LentaAPI:
    ...
    async def get_products(self, category_id: int = DEFAULT_CATEGORY_ID, limit: int = DEFAULT_LIMIT) -> List[Dict]:
        """Асинхронное получение списка товаров"""
        try:
            json_data = {
    ...
    async def get_product_details(self, product_id: int) -> Optional[Dict]:
        """Асинхронное получение детальной информации о товаре"""
        try:
...
class ProductParser:
    ...
    async def parse_product(self, product_data: Dict) -> Optional[Product]:
        """Асинхронный парсинг данных товара"""
        try:
            ...
            item_detail = await self.api.get_product_details(product_id)
...
class DataSaver:
    ...
    async def save_products(self, products: List[Product]) -> None:
        """Асинхронное сохранение товаров в CSV"""
        try:
...
@timer_decorator
async def process_products_async(products: List[Dict], parser: ProductParser) -> List[Product]:
    """Асинхронная обработка товаров"""
    logger.info(f"Начинаем асинхронную обработку {len(products)} товаров")
    ...
    async def process_with_semaphore(product):
        async with semaphore:
            return await parser.parse_product(product)
...
async def main():
    ...
    async with LentaAPI() as api:
        ...
        parsed_products = await process_products_async(products, parser)
        
        # Сохраняем результаты
        await saver.save_products(parsed_products)
